...
class MainActivity : AppCompatActivity() {
    ...
    // Optimization: Lazy initialization speeds up app launch time
    ...
    private fun showMenuOptions() {
        ...
        val options = arrayOf(
            ...
            "Add-ons Store", "Check for Updates", "Extensions Dashboard", "Clear Browsing Data"
        )
        ...
        AlertDialog.Builder(this).setTitle("Settings").setItems(options) { _, which ->
            when (which) {
                ...
                2 -> { createNewSession(); activeTab.session.loadUri("https://addons.mozilla.org/android/") }
                3 -> { extensionManager.checkForUpdates(); Toast.makeText(this, "Checking for updates...", Toast.LENGTH_SHORT).show() }
                4 -> { 
                ...
                5 -> { Toast.makeText(this, "Coming in Phase 5!", Toast.LENGTH_SHORT).show() }
            }
        }.show()
    }
    ...
    override fun onResume() { 
        ...
        extensionManager.checkForUpdates() // Silent background update check
    }

    override fun onDestroy() {
        super.onDestroy()
        // Optimization: Graceful Shutdown. Only kill the engine if the app is truly being swiped away.
        if (isFinishing) {
